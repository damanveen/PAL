...
include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pdf/class_1_period.php"); //gathers all classes in a period

for ($pdf_class_counter = 0; $pdf_class_counter < $class_1_count; $pdf_class_counter++) { 
   $pdf->AddPage(); //CREATES A NEW PAGE
   $sql = "SELECT wcss_sso.users.first_name, wcss_sso.users.last_name, wcss_pal.courses.course_code FROM wcss_sso.users 
           INNER JOIN wcss_pal.class ON wcss_pal.class.teach_id = wcss_sso.users.id 
           INNER JOIN wcss_pal.courses ON wcss_pal.courses.course_id = wcss_pal.class.course_id
           WHERE wcss_pal.class.class_id = $class_1[$pdf_class_counter]";
   include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pal-sql.php");   
  ...
  include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pdf/class_students.php"); //gathers all the student_id in a class
  ...
  foreach ($students as $item) {  
    ...
    $stud_id = $item;
    include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/student_report_cards.php"); //gather's all the class_id of a student
        ...
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . $picture_location) != 0) {
          ...
          $pdf->Image($_SERVER['DOCUMENT_ROOT'] . '/PAL/images/studentPics/Blank.jpg',145,8,33);
        }
        /***************************************************************************/
        ...
        for ($loop = 0; $loop < $class_counter; $loop++) { 
          ...
          include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pdf/student_report_info.php"); //GATHERS ALL THE STUDENT PROGRESS REPORT CARD INFORMATION


          $sql = "SELECT wcss_sso.users.first_name, wcss_sso.users.last_name FROM wcss_sso.users 
          		 INNER JOIN wcss_pal.class ON wcss_pal.class.teach_id = wcss_sso.users.id WHERE wcss_pal.class.class_id = $class_id";
  		    include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pal-sql.php"); //gather the teacher's name
