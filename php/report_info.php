...
include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/check_progress-report.php"); //checks if user has any existing progress report for the class
...
if ($report_valid == 1) { 
	...
	include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pal-sql.php"); //runs SQL command query
	$info = $results -> fetch(PDO::FETCH_ASSOC);

	include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/check_student-comments.php"); //gathers all existing comments for the student
	...
	include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pal-sql.php"); //runs SQL command 
	...
	include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pal-non_query-sql.php"); //runs SQL command 
	$sql = "INSERT INTO student_comments VALUES ('', '$Scomment_id', '')";
	include ($_SERVER['DOCUMENT_ROOT'] . "/PAL/php/pal-non_query-sql.php"); //runs SQL command 
	header("Location: http://dev.emmell.org/PAL/student_report.php?stud_id=$stud_id&class=$class_id"); //reloads the page so that the page will load the registered student's new report card information from the table report_student which had not existed before
}
?>